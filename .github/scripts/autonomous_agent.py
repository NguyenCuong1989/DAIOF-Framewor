...
class AutonomousAIAgent:
    ...
    def __init__(self):
        ...
        if not self.token:
            self.log_action(
                "⚠️  GITHUB_TOKEN not set - running in LOCAL DRY-RUN MODE", "WARNING"
            )
            self.gh = None
            self.repo = None
            self.dry_run = True
            ...
            for issue in issues:
                if len(list(issue.labels)) == 0:
                    ...
                    if labels:
                        ...
                        labeled_count += 1
                        self.log_action(
                            f"🏷️ Auto-labeled issue #{issue.number}: {labels}"
                        )
    ...
    def _welcome_new_contributors(self):
        ...
        try:
            ...
            for pr in pulls[:5]:  # Giới hạn 5 PRs gần nhất
                ...
                author_prs = list(
                    self.repo.get_pulls(state="all", creator=pr.user.login)
                )
...
"""
                        ...
                        welcomed_count += 1
                        self.log_action(
                            f"👋 Welcomed first-time contributor: @{pr.user.login}"
                        )
        ...
        try:
            ...
            for pr in pulls[:10]:  # Giới hạn 10 PRs
                if pr.merged and pr.merged_at and pr.merged_at > since:
                    ...
                    comments = list(pr.get_issue_comments())
                    already_thanked = any(
                        "Thank you for your contribution" in c.body for c in comments
                    )
    ...
    def _update_contributors(self):
        ...
        try:
            ...
            with open(contrib_file, "w") as f:
                ...
                f.write(
                    "Thank you to all the amazing people who have contributed to DAIOF Framework!\n\n"
                )

                for contrib in contributors:
                    contributions = contrib.contributions
                    f.write(
                        f"- [@{contrib.login}]({contrib.html_url}) - {contributions} commits\n"
                    )
...
"""
        ...
        report_file = (
            Path("metrics") / f'report_{datetime.utcnow().strftime("%Y%m%d")}.md'
        )
    ...
    def run(self):
        ...
        try:
            ...
            log_file = (
                Path("metrics")
                / f'agent_log_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.txt'
            )
