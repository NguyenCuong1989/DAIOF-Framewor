...
async function loadWidget(): Promise<string> {
  return readFile(resolve(here, '../web/widget.html'), 'utf8');
}
...
async function createMcpServer() {
  ...
  const server = new McpServer(
    { name: 'GPT Execution Canvas', version: '0.1.0' },
  ...
  server.registerTool('get_execution_run', {
    ...
    _meta: {
      'openai/toolInvocation/invoking': 'Loading execution evidence…',
      'openai/toolInvocation/invoked': 'Execution evidence loaded',
    },
  }, async ({ runId }) => {
  ...
  server.registerTool('render_execution_canvas', {
    ...
    _meta: {
      ...
      'openai/toolInvocation/invoking': 'Rendering execution canvas…',
      'openai/toolInvocation/invoked': 'Execution canvas ready',
    },
  }, async ({ run, events, approval }) => textResult(
    { run, events, ...(approval ? { approval } : {}) },
    `Rendered execution run ${String(run.id ?? '')}.`,
  ));
  ...
  server.registerTool('request_execution_approval', {
    ...
    _meta: {
      ...
      'openai/outputTemplate': TEMPLATE_URI,
      'openai/widgetAccessible': true,
      'openai/toolInvocation/invoking': 'Creating approval gate…',
      'openai/toolInvocation/invoked': 'Approval required',
    },
  }, async ({ runId, scope }) => {
  ...
  server.registerTool('decide_execution_approval', {
    ...
    _meta: {
      ui: { resourceUri: TEMPLATE_URI, visibility: ['app'] },
      'openai/outputTemplate': TEMPLATE_URI,
      'openai/widgetAccessible': true,
    ...
    const run = await store.getRun(approval.run_id);
    const events = await store.listEvents(approval.run_id);
  ...
  server.registerTool('advance_execution_run', {
    ...
    inputSchema: {
      ...
      stage: stageSchema,
      status: statusSchema,
      eventKind: z.string().min(1).max(96),
    ...
    _meta: {
      ...
      'openai/outputTemplate': TEMPLATE_URI,
      'openai/widgetAccessible': true,
      'openai/toolInvocation/invoking': 'Applying approved transition…',
      'openai/toolInvocation/invoked': 'Execution state advanced',
    },
  }, async (input) => {
...
app.get('/healthz', (_req, res) => res.json({ ok: true, service: 'gpt-execution-canvas', version: '0.1.0' }));
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(JSON.stringify({ level: 'info', event: 'server_started', port: PORT, origin: PUBLIC_ORIGIN }));
});
