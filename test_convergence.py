...
from autonomous_todo_system import (ConvergenceOptimizedTodoSystem, Task,
                                    TaskPriority)
...
def test_convergence_with_tasks():
    ...
    for cycle_num in range(1, 11):  # 10 cycles
        ...
        if cycle_num > 1:
            ...
            print(
                f"Cycle {cycle_num:2d}: D_k = {d_current:6.2f} | "
            ...
            print(
                f"Cycle {cycle_num:2d}: D_k = {complexity_history[-1]:6.2f} | "
    ...
    if violations == 0:
        print(
            f"  ✅ D_{{k+1}} ≤ D_k maintained for ALL {len(complexity_history)-1} transitions!"
        )
    ...
    print(
        f"📉 Reduction: {complexity_history[0] - complexity_history[-1]:.2f} "
