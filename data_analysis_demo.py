...
from digital_ai_organism_framework import (DigitalEcosystem, DigitalGenome,
                                           DigitalOrganism,
...
def analyze_with_dr_protocol(data, symphony_control):
    ...
    deconstructed = symphony_control.apply_dr_protocol(
        data, "data_analysis_deconstruction"
    )
    ...
    print(
        f"  Pillar Scores: Safety={focal['pillar_scores']['safety']:.2f}, "
...
def run_ecosystem_analysis(organisms, data, symphony_control):
    ...
    print(
        f"📊 Social connections established: {sum(len(org.social_connections) for org in organisms)}"
    )
...
def generate_insights(data, ecosystem_report, harmony_index, symphony_reflections):
    ...
    dau_trend = (
        "tăng"
        if user_data["daily_active_users"][-1] > user_data["daily_active_users"][0]
        else "giảm"
    )

    insights.append(
        f"📈 User Engagement: DAU trung bình {dau_avg:.0f}, xu hướng {dau_trend} 37% trong tuần"
    )
    insights.append(
        f"⚡ Session Duration: Cải thiện từ {user_data['session_duration'][0]} lên {user_data['session_duration'][-1]} phút"
    )
    ...
    insights.append(
        f"🚀 System Performance: Response time trung bình {response_avg:.1f}s, CPU usage {cpu_avg:.1f}%"
    )
    insights.append(
        f"📊 Error Rate: Giảm từ {sys_data['error_rate'][0]*100}% xuống {sys_data['error_rate'][-1]*100}%"
    )

    # Business Insights
    biz_data = data["business_metrics"]
    revenue_growth = (
        (biz_data["revenue"][-1] - biz_data["revenue"][0]) / biz_data["revenue"][0]
    ) * 100

    insights.append(
        f"💰 Business Growth: Revenue tăng {revenue_growth:.1f}% trong tuần"
    )
    insights.append(
        f"⭐ Customer Satisfaction: Đạt {biz_data['customer_satisfaction'][-1]}/5.0"
    )

    # AI-Generated Insights from Ecosystem
    insights.append(f"🧬 Ecosystem Harmony: {harmony_index:.3f}/1.0")
    insights.append(
        f"🤔 Socratic Reflections: {len(symphony_reflections)} insights generated"
    )
...
def main():
    ...
    ecosystem, ecosystem_report, final_harmony = run_ecosystem_analysis(
        organisms, data, symphony_control
    )

    # Tạo insights
    insights = generate_insights(
        data, ecosystem_report, final_harmony, symphony_control.socratic_reflections
    )
    ...
    analysis_results = {
        ...
        "symphony_reflections": [
            r["question"] for r in symphony_control.socratic_reflections
        ],
        "creator_attribution": {
            "creator": "Nguyễn Đức Cường (alpha_prime_omega)",
            "framework": "HYPERAI",
