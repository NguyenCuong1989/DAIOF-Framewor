...
class HealthChecker:
    ...
    def check_imports(self):
        ...
        try:
            from src.hyperai import (DigitalEcosystem, DigitalOrganism,
                                     HAIOSCore, SymphonyControlCenter)
    ...
    def check_tests(self):
        ...
        if len(test_files) < 2:
            self.warnings.append(
                f"Only {len(test_files)} test files found (expected 2+)"
            )
    ...
    def check_git_health(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "status"], cwd=project_root, capture_output=True, timeout=10
            )
            if result.returncode == 0:
                self.passed_checks += 1
